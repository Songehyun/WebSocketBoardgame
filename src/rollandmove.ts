document.addEventListener('DOMContentLoaded', () => {
  ...
  const currentPlayerDisplay = document.getElementById('current-player-display') as HTMLParagraphElement;
  ...
  function saveGameState() {
    const gameState = {
      ...
      currentPlayerRect,
      extraRoll,
      pieces: {} as { [key: string]: { parentId: string; playCount: string | null } },
    };
  ...
  rollButton.addEventListener('click', () => {
    ...
    const diceResultImg = document.getElementById('dice-result') as HTMLImageElement;
  ...
  function moveToStartPosition(player: number, piece: HTMLElement, roll: number) {
    ...
    if (startRect.childElementCount > 0) {
      const occupyingPlayer = (startRect.firstChild as HTMLElement)?.getAttribute('data-player');
  ...
  function movePlayer(player: number, roll: number) {
    ...
    const piecePlayCount = parseInt(currentPlayerPiece!.getAttribute('data-playcount') || '0', 10);
  ...
  function moveToDestination(player: number, steps: number) {
    ...
    const destination = document.getElementById(destinations[steps - 1]) as HTMLElement;
  ...
  function movePieceToNewRect(position: number, player: number, pieceNumber: string, initialRect: number) {
    const newRect = document.getElementById(`rect${position}`) as HTMLElement;

    if (newRect.childElementCount > 0) {
      const firstChildPlayer = (newRect.firstChild as HTMLElement)?.getAttribute('data-player');
      if (firstChildPlayer !== String(player)) {
        moveToNest(parseInt(firstChildPlayer!, 10), newRect.firstChild as HTMLElement);
        extraRoll = true;
      }
    }
